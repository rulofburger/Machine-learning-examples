...
for(i in 1:11) seeds[[i]] <- sample.int(1000, 1)
fitControl <- trainControl(method = "cv", number = 10, verboseIter = TRUE, allowParallel = TRUE,seeds=seeds)
rpart.fit.time = system.time(rpart.fit <- train(formula, data=training, method = 'rpart', control= rpart.control(minsplit = 69, maxdepth = 28),tuneGrid=expand.grid(cp=7.41e-05)))
